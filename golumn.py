"""
Package up basic running of a GOTM water column
"""
import os
import pandas as pd
import xarray as xr
...
class Golumn: # GOTM water column
    rouse_base="../gotm/cases/rouse"
    run_dir="../gotm/rouse_up"
    debug=False
...
location:
   name: Rouse profile                     # station name used in output [default=GOTM site]
...
time:
   ...
   stop: 1998-01-02 00:00:00               # stop date and time [yyyy-mm-dd HH:MM:SS; default=2018-01-01 00:00:00]
   dt: 36.0                                # time step for integration [s; min=0.0; default=3600.0]
grid:
...
temperature:                               # temperature profile used for initialization and optionally relaxation [Celsius]
   method: off                             # method [off, file=from file, constant, two_layer=two layers with linear gradient in between, buoyancy=from salinity and buoyancy frequency; default=off]
   ...
   type: in-situ                           # temperature measure [in-situ, potential, conservative; default=in-situ]
   two_layer:
      z_s: 30.0                            # depth where upper layer ends [m; min=0.0; default=0.0]
      t_s: 20.0                            # upper layer temperature [Celsius; min=-2.0; max=40.0; default=0.0]
      z_b: 40.0                            # depth where lower layer begins [m; min=0.0; default=0.0]
      t_b: 15.0                            # lower layer temperature [Celsius; min=-2.0; max=40.0; default=0.0]
...
salinity:                                  # salinity profile used for initialization and optionally relaxation [psu]
   method: off                             # method [off, file=from file, constant, two_layer=two layers with linear gradient in between, buoyancy=from temperature and buoyancy frequency; default=off]
   constant_value: 20.0                    # value to use throughout the simulation [psu; min=0.0; default=0.0]
   file:                                   # path to file with series of profiles [default=]
   column: 1                               # index of column to read from [default=1]
   type: practical                         # salinity measure [practical, absolute; default=practical]
   two_layer:
      z_s: 30.0                            # depth where upper layer ends [m; min=0.0; default=0.0]
...
surface:
   ...
   swr:                                    # shortwave radiation [W/m^2]
      method: constant                     # method [constant, file=from file, calculate=from time, location and cloud cover; default=constant]
   ...
   longwave_radiation:                     # net longwave radiation [W/m^2]
      ...
      column: 1                            # index of column to read from [default=1]
   albedo:
      method: constant                     # method to compute albedo [constant, payne=Payne (1972), cogley=Cogley (1979); default=payne]
      constant_value: 0.0                  # constant value to use throughout the simulation [fraction; min=0.0; max=1.0; default=0.0]
   ice:
      model: no_ice                        # model [no_ice=no ice model, simple=freezing temperature, basal_melt=Basal Melt , lebedev=Lebedev (1938), mylake=Saloranta and Andersen (2007), winton=Michael Winton (2000); default=simple]
...
mimic_3d:                                  # effects of horizontal gradients
   ext_pressure:                           # external pressure
      ...
      dpdx:                                # pressure in West-East direction
         ...
         constant_value: 1.00000000E-06    # value to use throughout the simulation [default=0.0]
         file:                             # path to file with time series [default=]
         column: 1                         # index of column to read from [default=1]
...
output:
   rouse:                                  # path of output file, excluding extension
      time_unit: dt                        # time unit [second, hour, day, month, year, dt=model time step; default=day]
...
"""
    ...
    def __init__(self,**kw):
        ...
        self.ryaml=YAML(typ='rt') 
        self.ryaml.default_flow_style = False
        self.init_rouse()
    
    def read_yaml(self,fn):
        with open(fn,'rt') as fp:
            return self.ryaml.load(fp)
            
    def write_yaml(self,data,fn):
        buff=io.StringIO()
        self.ryaml.dump(data,buff)
        patched=re.sub(r"'(true|false)'",r"\1",buff.getvalue())
        with open(fn,'wt') as fp:
            fp.write(patched)

    def init_rouse(self):
        self.gotm_yaml = self.read_yaml(os.path.join(self.rouse_base,"gotm.yaml"))
        self.fabm_yaml = self.read_yaml(os.path.join(self.rouse_base,"fabm.yaml"))
    ...
    def prep_run(self):
        ...
        fn=os.path.join(self.run_dir,'gotm.yaml')
        self.write_yaml(self.gotm_yaml, fn)
        print("Just wrote yaml config to ",fn)
        
        self.write_yaml(self.fabm_yaml, os.path.join(self.run_dir,'fabm.yaml'))
    def run_gotm(self):
        if os.path.exists(self.nc_fn):
            ds=xr.open_dataset(self.nc_fn, drop_variables=['z','zi'])
            ds.close() # avoid caching
            os.unlink(self.nc_fn) # failed run detected by no output
        self.gotm_output=None
        with utils.working_dir(self.run_dir):
            try:
                if self.debug:
                    self.gotm_output = subprocess.check_output("gotm")
                else:
                    self.gotm_output = subprocess.check_output("gotm",stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as exc:
                print("Error with GOTM run:")
                print(exc.output)
                raise
    @property
    def nc_fn(self):
        return os.path.join(self.run_dir, "rouse.nc")
        
    def post_run(self):
        ds=xr_utils.xr_open_with_rename(self.nc_fn,dict(z="z_coord",zi="zi_coord"))
        self.results=ds.isel(lat=0,lon=0)

    def fig_time_z(self,fields,column=False):
        """
        column: arrange plots in a column instead of a row 
        """
        rows=1 ; cols = len(fields)
        if column:
            rows,cols = cols,rows
        fig,axs=plt.subplots(rows,cols,sharex=True,sharey=True,figsize=(9.5,4.5),squeeze=False)
        for f,ax in zip(fields,axs.ravel()):
            scal = self.results[f]
            if 'z' in scal.dims:
                t,z = xr.broadcast(self.results.time, self.results.z_coord)
            else: 
                t,z = xr.broadcast(self.results.time, self.results.zi_coord)

            coll=ax.pcolormesh(t, z, scal, cmap='turbo')
            plt.colorbar(coll,label=f)
    ...
    def write_file(self,da,name=None):
        if name is None:
            name=da.name
            assert name
        base = f"{da.name}_file.dat"
        fn=os.path.join(self.run_dir,base)
        with open(fn,'wt') as fp:
            if len(da.dims)==1: # assume time series
                # e.g.
                # 1976-04-06 06:00:00   -2.122162e+02
                fp.write( da.to_pandas().to_string(header=False) )
            elif len(da.dims)==2: # assume time-varying profile, with time and z coordinates
                for ti in range(da.shape[0]): # assumes time is first
                    profile = da.isel(time=ti)
                    t_str = utils.strftime(profile.time, "%Y-%m-%d %H:%M:%S")
                    fp.write(f"{t_str}  {profile.shape[0]} 1\n")
                    fp.write( pd.DataFrame( dict(z=profile.z,salinity=profile.values))
                                .to_string(header=False,index=False) )
                    fp.write("\n")
                
        return base # relative to self.run_dir
